...
class SS_Uploader(BasicScraper):
    def __init__(self, url, headless=False, log_file="scraper.log", output_dir=None):
        super().__init__(url, headless=headless)
        self.driver = None
        self.output_dir = output_dir if output_dir else os.path.join(get_base_dir(), "data")
    ...
    '''def main_upload(self, upload_description):
        authenticate(self, By.CSS_SELECTOR, MyHomeUploadSelectors.AUTHENTICATION_BUTTON,
                     ...
                     self.myhome_authentication_data["SSGE"]["email"],
                     self.myhome_authentication_data["SSGE"]["password"])'''

    def run_upload(self, ad_id, enter_description=True, stop_event=None):
        data_folder = os.path.join(self.output_dir, ad_id)
        ...
        try:
            ...
            if not SS_Uploader.click_element(self.driver, login_locator, stop_event=stop_event):
                self.driver.quit()
                return None
            if not SS_Uploader.send_keys_to_element(self.driver, (By.NAME, "email"), self.myhome_authentication_data["SSGE"]["email"], stop_event=stop_event):
                self.driver.quit()
                return None
            if not SS_Uploader.send_keys_to_element(self.driver, (By.NAME, "password"), self.myhome_authentication_data["SSGE"]["password"],
                                                    stop_event=stop_event):
                self.driver.quit()
                return None
            ...
            if os.path.exists(data_folder):
                image_paths = [os.path.abspath(os.path.join(data_folder, "images", img))
                               for img in os.listdir(os.path.join(data_folder, "images"))
            ...
            def rewrite_location_temqa(text):
                match = re.match(r"(.*?)\s*-\s*([XVI]+)\s*მ/რ\s*([IVXLCDM]+)\s*კვარტ\.", text)
                if match:
                    area, micro, quarter = match.groups()
                    return f"{micro} მიკრორაიონი, {quarter} კვარტალი - {area}"
                return text
            location = data.get("location", "")
            if "თემქა" in location:
                location = rewrite_location_temqa(location)
            else:
                pass
...
if __name__ == "__main__":
    ...
    final_url = uploader.run_upload(ad_id="20445069", enter_description=True, stop_event=None)
